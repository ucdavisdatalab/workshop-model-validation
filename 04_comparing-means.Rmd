...
paste( mean( (preds[[1]] - cars$dist)**2, na.rm=TRUE ), 
...
with(covid[!train, ], plot(date, D1_admissions, type='l'))

# plot the predictions over the validation period
lines( covid$date[ !train ], pred_covid, col='red')

# calculate the mean absolute error over the validation period:
cat( "model MAE=", mean( abs( pred_covid - covid$D1_admissions[ !train ])))

# compare to the MAE of carrying forward the most recent observation:
cat( "lag MAE=",
       mean( abs( with(covid, COVID_NEW_ADM_CNT - D1_admissions)[ !train ])))
```
